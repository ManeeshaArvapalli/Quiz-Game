...
TOPICS_LIST = ['commerce', 'history', 'science', 'technology', 'worldgk']
...
def load_question(filename):
    ...
    try:
        ...
        # Optionally handle this error, e.g., return None or raise an exception
...
def play_quiz():
    flag = False
    try:
        choice = int(input("Welcome to Today's Quiz!\nChoose your domain of interest:Science\n(3). History of India\n(2). Commerce\n(1). Technology\n(4). World Gk(5)\nEnter Your Choice [1/2/3/4/5]: "))
        if choice > len(TOPICS_LIST) or choice < 1:
            print("Invalid Choice. Enter Again")
            flag = True # raising flag
    except ValueError as e:
    ...
    if not flag:
        filename =TOPICS_LIST[choice-1]+".json"
        print(f"Attempting to load file: {filename}")
        questions = load_question(filename)
        if questions:
            test(questions)
        else:
            print("Quiz cannot be started due to missing or invalid questions data.")
    else:
        play_quiz() # replay if flag was raised
